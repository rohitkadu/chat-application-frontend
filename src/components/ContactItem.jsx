...
const ContactItem = ({ contact, onSelectContact, onModalAction, selectedUser, ucc }) => {
  ...
  return (
    <div className={`contact-item ${isSelected ? 'active' : ''}`} onClick={() => onSelectContact(contact.userId)}>
      ...
      {(contact.unreadCount > 0 && !isSelected) && <span>{contact.unreadCount}</span>}
      {isOddUcc && isSelected && <span>{contact.unreadCount}</span>}
